...
namespace AbsenCoordinateMobile
{
    public class Account
    {
        ...
        internal static async Task SetProfile(Karyawan response)
        {
            var userString = JsonConvert.SerializeObject(response);
            await SecureStorage.SetAsync("Profile", userString);
        }

        public static async Task<Karyawan> GetProfile()
        {
            var userString = await SecureStorage.GetAsync("Profile");
            if (string.IsNullOrEmpty(userString))
                return null;
            else
                return JsonConvert.DeserializeObject<Karyawan>(userString);
        }
